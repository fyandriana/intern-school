...
export function getCourseByIdSvc(id) {
  ...
  const quizCount = countQuizzesForCourse(id);

  if (!course) throw new NotFoundError("Course not found", { id });
  return { ...course, quizCount, hasQuizzes: quizCount > 0 };
}
...
export function getCoursesCountSvc({ teacherId } = {}) {
  try {
    const total = teacherId != null ? countCoursesByTeacher(teacherId) : countCourses();
    return { total };
  } catch (err) {
